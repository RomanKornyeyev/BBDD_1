...
SELECT AL.APENOM, A.NOMBRE, N.NOTA FROM ALUMNOS AL, ASIGNATURAS A, NOTAS N WHERE AL.DNI=N.DNI AND A.COD=N.COD;

/*•	Seleccionar el apellido, el oficio y la localidad de los departamentos donde trabajan los ANALISTAS.*/
SELECT APELLIDO, DNOMBRE, LOC FROM EMPLE E, DEPART D WHERE E.DEPT_NO=D.DEPT_NO AND OFICIO='ANALISTA';
...
SELECT TO_CHAR(SUM(SALARIO), '999G999D99L') "SUMA SALARIOS" FROM EMPLE;
...
SELECT
	TO_CHAR(AVG(SALARIO), '999G999D99L') "MEDIO",
	COUNT(COMISION) "COMISIONES",
	TO_CHAR(MAX(SALARIO), '999G999D99L') "MAX",
	TO_CHAR(MIN(SALARIO), '999G999D99L') "MIN"
FROM
	EMPLE
WHERE
	DEPT_NO=30;
...
SELECT DEPT_NO "DEPART", COUNT(EMP_NO) "NUM EMPLE" FROM EMPLE GROUP BY DEPT_NO;

/*•	Visualizar los departamentos con más de 4 empleados*/
SELECT DEPT_NO "DEPART", COUNT(EMP_NO) "NUM EMPLE" FROM EMPLE GROUP BY DEPT_NO HAVING COUNT(EMP_NO) > 4; 

/*•	Obtener los nombres de los departamentos que tengan más de tres personas trabajando*/
/* repasar */
SELECT DNOMBRE, COUNT(EMP_NO) FROM EMPLE E, DEPART D WHERE E.DEPT_NO=D.DEPT_NO GROUP BY E.DEPT_NO, D.DNOMBRE HAVING COUNT(EMP_NO) > 3;
...
INSERT INTO EMPLE (SELECT 1113, 'QUIROGA', OFICIO, DIR, SYSDATE, SALARIO, COMISION, DEPT_NO FROM EMPLE WHERE APELLIDO='GIL');
...
SELECT NOMBRE FROM ALUM WHERE NOMBRE NOT IN (SELECT NOMBRE FROM ANTIGUOS);
/**** MINUS = NOT IN ****/