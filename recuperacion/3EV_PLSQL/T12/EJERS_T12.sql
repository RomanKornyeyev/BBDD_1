...
BEGIN
	...
	IF (PREC >= V_PRECIO*0.98) AND (PREC <= V_PRECIO*1.02) THEN
		UPDATE ARTICULOS SET PRECIO_COSTO=PREC WHERE ARTICULO=NOM AND CATEGORIA=CAT;
		DBMS_OUTPUT.PUT_LINE('PRECIO ANTIGUO: '||V_PRECIO||', PRECIO NUEVO: '||PREC);
	ELSE
		DBMS_OUTPUT.PUT_LINE('Precio superior al 2%');
	END IF;
EXCEPTION
	WHEN NO_DATA_FOUND THEN
